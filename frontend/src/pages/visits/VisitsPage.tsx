            ...
            <div className="mt-4 space-y-2 text-sm text-gray-600">
             ...
             <div className="mt-4 pt-4 border-t flex space-x-2">
                {visit.status === VisitStatus.PENDING && (
                    <button onClick={() => onApprove(visit._id)} className="px-3 py-1 text-xs font-semibold text-white bg-blue-500 rounded hover:bg-blue-600">Aprobar</button>
                )}
                {visit.status === VisitStatus.APPROVED && (
                    <button onClick={() => onCheckIn(visit._id)} className="px-3 py-1 text-xs font-semibold text-white bg-green-500 rounded hover:bg-green-600">Check-in</button>
                )}
                 {visit.status === VisitStatus.CHECKED_IN && (
                    <button onClick={() => onCheckout(visit)} className="px-3 py-1 text-xs font-semibold text-white bg-red-500 rounded hover:bg-red-600">Check-out</button>
                )}
